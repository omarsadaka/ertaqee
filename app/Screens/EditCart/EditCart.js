...
class EditCart extends Component {
  ...
  componentWillMount() {
    ...
    let ParamsData = this.props.route.params.Item;
    if (ParamsData.includes_me) {
      this.setState({includes_me: true});
    } else {
      this.setState({includes_me: false});
    }
    console.log('ParamsData.Item', ParamsData);
    const list = ParamsData.related_models;
    console.log('ParamsData.Item.related_models', list);
    const data = [];
    ...
    this.setState({course_id: ParamsData.model.id});
        ...
        <View style={{flexDirection: 'row'}}>
          <View>
            <View
              ...
              <Text style={{textAlign: 'left', fontSize: hp('2%')}}>
                {' '}
                {name}{' '}
              </Text>
            </View>
  ...
  MyItems() {
    ...
    for (let i = 0; i < user.length; i++) {
      Items.push(
        this.userItem(i, user[i].name, user[i].mobile, user[i].email, false),
      );
      arr.push(i);
    }
    Items.push(
      ...
      <View
        ...
        <View
          ...
          <TouchableOpacity
            style={[
              styles.LoginBTNStyle,
              {height: hp('7%'), width: wp('80%'), marginTop: hp('1%')},
            ]}
            onPress={() => this.AdduserData()}>
            <Text style={styles.NextTxt}>{strings.add}</Text>
          </TouchableOpacity>
  ...
  AddToBasket() {
    console.log('aasddddd', this.state.user);
        ...
        response
          ...
          .then(responseJson => {
            ...
            if (responseJson.success) {
              ...
              console.log("🚀 ~ file: EditCart.js:592 ~ EditCart ~ AddToBasket ~ responseJson.errors:", responseJson.errors)
              if (responseJson.errors[0] === 'Item already exist in cart.') {
                // alert(responseJson.errors[0]);
                alert(strings.already_exsist);
              } else {
                alert(responseJson.errors[0]);
              }
            }
    ...
    return (
      <SafeAreaView>
        <ScrollView style={{backgroundColor: '#FFFFFF'}}>
          ...
          {/* {ShowEditUser && this.EditUserForm()} */}
        </ScrollView>
      </SafeAreaView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  NextTxt: {fontSize: hp('2.2%'), color: 'white'},
  InputStyle: {
    borderColor: 'silver',
    margin: 2,
    borderColor: 'silver',
